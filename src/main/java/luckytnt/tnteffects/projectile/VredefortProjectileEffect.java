...
public class VredefortProjectileEffect extends PrimedTNTEffect {
	...
	public void serverExplosion(IExplosiveEntity ent) {
		...
		for(int count = 0; count < 300; count++) {
			LExplosiveProjectile projectile = EntityRegistry.SOLAR_ERUPTION_PROJECTILE.get().create(ent.level());
		...
		for(int count = 0; count < 6; count++) {
			LExplosiveProjectile projectile = EntityRegistry.LITTLE_METEOR.get().create(ent.level());
			projectile.setPos(ent.getPos());
			projectile.setOwner(ent.owner());
	...
	public void spawnParticles(IExplosiveEntity ent) {
		ent.level().addParticle(ParticleTypes.EXPLOSION, ent.x(), ent.y(), ent.z(), 0, 0, 0);
	}
