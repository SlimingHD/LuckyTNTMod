		...
		if (entity instanceof LExplosiveProjectile ent) {
			if (ent.getTNTFuse() == 0) {
				if (!level.isClientSide()) {
					entity.getLevel().playSound(null, toBlockPos(entity.getPos()), SoundEvents.GENERIC_EXPLODE, SoundSource.BLOCKS, 4f, (1f + (level.getRandom().nextFloat() - level.getRandom().nextFloat()) * 0.2f) * 0.7f);
					serverExplosion(entity);
				}
				ent.destroy();
			}
			if (ent.inGround() || ent.hitEntity() || ent.getPersistentData().getBoolean("hitBefore")) {
