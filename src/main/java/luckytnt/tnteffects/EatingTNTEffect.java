...
public class EatingTNTEffect extends PrimedTNTEffect{
	...
	public void serverExplosion(IExplosiveEntity entity) {
		ImprovedExplosion explosion = new ImprovedExplosion(entity.level(), entity.getPos(), 10f + (float)Math.floor((1f/7.5f) * entity.getPersistentData().getInt("eatLevel")));
	...
	public void explosionTick(IExplosiveEntity entity) {
		...
		for(ItemEntity item : items) {
			...
			if(entity.getPos().distanceTo(item.getPosition(1)) < 1) {
				entity.getPersistentData().putInt("eatLevel", entity.getPersistentData().getInt("eatLevel") + item.getItem().getCount());
