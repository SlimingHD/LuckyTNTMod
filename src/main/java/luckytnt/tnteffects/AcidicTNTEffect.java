	...
	public void explosionTick(IExplosiveEntity ent) {
		if(ent instanceof PrimedLTNT) {
			...
			if(ent.getTNTFuse() % 20 == 0) {
				List<LivingEntity> list = ent.getLevel().getEntitiesOfClass(LivingEntity.class, new AABB(toBlockPos(ent.getPos()).offset(-3, -3, -3), toBlockPos(ent.getPos()).offset(3, 3, 3)));			
