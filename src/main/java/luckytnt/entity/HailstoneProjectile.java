...
public class HailstoneProjectile extends LExplosiveProjectile {
	...
	public void onHitBlock(BlockHitResult result) {
		...
		level().playSound(null, blockPosition(), SoundEvents.GLASS_BREAK, SoundSource.BLOCKS, 0.5f, 1f);
	...
	public void onHitEntity(EntityHitResult result) {
		...
		if (!level().isClientSide()) {
			result.getEntity().hurt(LuckyTNTDamageSources.hailstone(level(), this, owner()), 4f);
		}
	} 
}
